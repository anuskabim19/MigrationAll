        ...
        BEGIN TRY
            ...
            BEGIN
				...
				SELECT
                  ...
                  + ' Employee Name: ' + ISNULL (ed.Name, '') , 255)) AS Example --select *
				FROM
				...
				PRINT @@ROWCOUNT;
--IF OBJECT_ID('tempdb..#ExampleEmployeeData') IS NOT NULL
                ...
                INSERT INTO #Mappingdata ( MappingId ,
                            ...
                            SELECT   @MappingId ,
                                   ...
                                   MAX ( LEFT('EmployeeID: ' + CONVERT (VARCHAR (255), er.EmployeeID)
                                    + ' Employee Name: ' + ISNULL (er.Name, '') , 255)) AS Example --select *
...
---------------------------------------------------------------------------------------
                ...
                PRINT 'Customer_WeekendDay';--weekday

                SELECT @MappingId = NULL;
                SELECT @MappingName = 'Customer_WeekendDay';
                SELECT @MappingId = ( SELECT MappingId
                                      FROM   dbo.Mapping
                                      WHERE  Mapping = 'Customer_WeekendDay'
                                      AND    MappingTypeId = @MappingTypeId );
...
---------------------------------------------------------------------------------------------------
                ...
                INSERT INTO #Mappingdata ( MappingId ,
                            ...
                            SELECT   @MappingId ,
                                     ...
                                     GETDATE () AS InsertDate,
									 MAX ( LEFT('CustomerID: ' + CONVERT (VARCHAR (255), cd.CustomerID)
                                              + ' Customer Name: ' + ISNULL (cd.CustomerName, '') , 255)) AS Example-- select * 
									

                                 FROM
    [Anuska_2024_new].dbo.Job j
	left join [Anuska_2024_new].dbo.jobtitle jt on jt.ID=j.JobID
...
--------------------------------------------------------------------------------------
                ...
                INSERT INTO #Mappingdata ( MappingId ,
                            ...
                            SELECT   @MappingId ,
                                     @MappingName , --select
                                     NULL AS Mapfromid ,
                                     LTRIM (RTRIM (edd.DeductionType)) AS Mapfrom ,
                                     LTRIM (RTRIM (edd.DeductionType)) AS Data1 ,
                                     COUNT (*) AS Data2 ,
                                     3 AS UserPersonId ,
                                     GETDATE () AS InsertDate,
									 MAX ( LEFT('EmployeeID: ' + CONVERT (VARCHAR (255), ed.EmployeeID)
	...
	left join [Anuska_2024_new].dbo.EmployeeData ed on ed.EmployeeID= edd.EmployeeID
                            ...
                            WHERE   
                                  ed.MigrationStatus = 'Migrate'
                            GROUP BY edd.DeductionType
,
                                      LTRIM (RTRIM (edd.DeductionType
))
                            ORDER BY  LTRIM (RTRIM (edd.DeductionType
));
                ...
                INSERT INTO #Mappingdata ( MappingId ,
                            ...
                            FROM     [anuska_2024_new].dbo.EmployeeTax ep
                                     ...
                                     LEFT JOIN dbo.MappingData md ON  md.Data1 = ep.taxname
                                                                  AND md.MappingId = 20443--@MappingId
                            WHERE    md.MappingDataId IS NULL
                            AND      e.Migrationstatus = 'Migrate'
							--AND    NOT EXISTS (
       --    SELECT 1
       --    FROM   dbo.MappingData md2
       --    WHERE  md2.MappingId = 20443--@MappingId
       --    AND    md2.MapFrom = ep.TaxName
       --    AND    md2.Data1 = ep.TaxName
       --)
GROUP BY ep.TaxName;
